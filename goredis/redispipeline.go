...
func (p *RedisPipeline) Cmd(ctx context.Context, args ...interface{}) *RedisCommond {
	...
	_ = p.Pipeliner.Process(ctx, redisCmd)
	return redisCmd
}

// 管道结合脚本，管道先按evalsha执行，管道中所有命令执行完之后，如果有脚本未加载的错误就再执行一次，所以这种管道无法保证命令顺序
func (p *RedisPipeline) Script(ctx context.Context, script *RedisScript, keys []string, args ...interface{}) *RedisCommond {
	...
	redisCmd := &RedisCommond{
		Cmd: redis.NewCmd(ctx, param...),
		ctx: ctx,
		nscallback: func() *redis.Cmd {
			// 直接同步调用
			return script.script.Eval(ctx, p.r, keys, args...) // 这里填充的是args
		},
	}
...
func (p *RedisPipeline) HMGetObj(ctx context.Context, key string, v interface{}) error {
	...
	redisCmd := &RedisCommond{
		...
		callback: func(reply interface{}) error {
			return ReplyToValues(reply, sInfo.Elemts)
		},
	}
	return p.Pipeliner.Process(ctx, redisCmd)
}
...
func (p *RedisPipeline) SetJson(ctx context.Context, key string, v interface{}) error {
	...
	if err != nil {
		utils.LogCtx(log.Error(), ctx).Err(err).Msg("RedisPipeline SetJson Param error")
		return err
	}
...
func (p *RedisPipeline) HSetJson(ctx context.Context, key, field string, v interface{}) error {
	b, err := json.Marshal(v)
	if err != nil {
		utils.LogCtx(log.Error(), ctx).Err(err).Msg("RedisPipeline HSetJson Param error")
		return err
	}
