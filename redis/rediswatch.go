...
func (r *Redis) WatchServices(key string, serverNames []string, fun func(infos []*RegistryInfo)) {
	...
	go func() {
		...
		for {
			...
			// 等待逻辑
			if subscriber != nil {
				reply := subscriber.ReceiveWithTimeout(0)
				...
				case redis.Pong:
					//fallthrough
				case redis.Message:
					log.Debug().Str("reason", string(msg.Data)).Msg("Redis will ReadServices")
				case error:
					// 出错误后，内部conn会关闭掉，不能重复使用了
					subscriber.Close()
					subscriber = nil
					log.Error().Err(msg).Msg("Redis Subscribe Receive fail")
				default:
					subscriber.Close()
					subscriber = nil
...
func (r *Redis) WatchServices2(key string, serverNames []string, fun func(addInfos, delInfos []*RegistryInfo)) {
	...
	go func() {
		...
		for {
			...
			// 等待逻辑
			if subscriber != nil {
				reply := subscriber.ReceiveContext(ctx)
				switch msg := reply.(type) {
				case redis.Subscription:
					//fallthrough
				case redis.Pong:
...
// 创建一个订阅对象，并判断订阅成功了
func (r *Redis) createWaitReadServicesSub(ctx context.Context, key string) (*redis.PubSubConn, error) {
	// 创建订阅者
	subscriber := &redis.PubSubConn{Conn: r.pool.Get()}
	// 订阅频道
	channel := fmt.Sprintf(regChannelFmt, key)
	err := subscriber.Subscribe(channel)
	if err != nil {
		subscriber.Close()
...
var readRegisterScirpt = NewScript(1, `
...
func (r *Redis) ReadServices(ctx context.Context, key string, serverNames []string) ([]*RegistryInfo, error) {
	...
	result, err := r.DoScriptStrings(ctx, readRegisterScirpt, key, RegExprieTime*1000)
	if err != nil {
		// 错误了 在来一次
		result, err = r.DoScriptStrings(ctx, readRegisterScirpt, key, RegExprieTime*1000)
		if err != nil {
...
func (r *Redis) checkServicesChange(ctx context.Context, key string, uuid string) bool {
	...
	ok, err := r.DoScriptInt(ctx, checkServicesScirpt, key, listKey, lockKey, RegExprieTime*1000, channel, uuid)
	if err != nil || ok == 0 {
		return false
	}
	return true
