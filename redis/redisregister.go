...
// ARGV2 发布订阅channel
// ARGV3... 添加的服务器
var registerScirpt = NewScript(2, `
...
// ARGV2 发布订阅channel
// ARGV3... 添加的服务器
var deregisterScirpt = NewScript(2, `
	redis.replicate_commands()
	-- 读取时间
	local t = redis.call('TIME')
	local stamp = tonumber(t[1])*1000 + tonumber(t[2])/1000
...
// ARGV2 发布订阅channel
// ARGV3 检查抢占的锁的UUID
var checkServicesScirpt = NewScript(3, `
	redis.replicate_commands()
	-- 先抢占下
...
func (r *Register) zadd() error {
	...
	_, err := r.r.DoScript(ctx, registerScirpt, args...)
	if err != nil {
		// 错误了 在来一次
		_, err = r.r.DoScript(ctx, registerScirpt, args...)
	}
...
func (r *Register) zrem() error {
	...
	_, err := r.r.DoScript(ctx, deregisterScirpt, args...)
	if err != nil {
		// 错误了 在来一次
		_, err = r.r.DoScript(ctx, deregisterScirpt, args...)
	}
