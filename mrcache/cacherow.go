...
func NewCacheRow[T any](redis *goredis.Redis, mysql *mysql.MySQL, tableName string, tableCount, tableIndex int) *CacheRow[T] {
...
func (c *CacheRow[T]) Get(ctx context.Context, cond TableConds) (*T, error) {
	...
	// 预加载 尝试从数据库中读取
	preData, _, err := c.preLoad(ctx, cond, key, nil)
	if err != nil {
		return nil, err
	}
	// 预加载的数据就是新增的
...
// 使用该函数需要提前使用ConfigOneCondField配置相关内容
// 见AddM说明
func (c *CacheRow[T]) AddMOC(ctx context.Context, condValue interface{}, data map[string]interface{}) (*T, interface{}, error) {
	if ctx.Value(utils.CtxKey_caller) == nil {
		ctx = context.WithValue(ctx, utils.CtxKey_caller, utils.GetCallerDesc(1))
	}
	if len(c.oneCondField) == 0 {
...
// 返回值：是T结构类型的指针(最新的值), 自增ID(如果新增数据 int64类型), error
func (c *CacheRow[T]) AddM(ctx context.Context, cond TableConds, data map[string]interface{}) (*T, interface{}, error) {
	if ctx.Value(utils.CtxKey_caller) == nil {
		ctx = context.WithValue(ctx, utils.CtxKey_caller, utils.GetCallerDesc(1))
	}
	// 检查data数据
...
func (c *CacheRow[T]) _Add(ctx context.Context, cond TableConds, dataInfo *utils.StructInfo) (*T, interface{}, error) {
	// 检查条件变量
	key, err := c.checkCond(cond)
	if err != nil {
		return nil, nil, err
...
// 使用该函数需要提前使用ConfigOneCondField配置相关内容
// 见Add2说明
func (c *CacheRow[T]) Add2OC(ctx context.Context, condValue interface{}, data interface{}) (interface{}, error) {
	if ctx.Value(utils.CtxKey_caller) == nil {
		ctx = context.WithValue(ctx, utils.CtxKey_caller, utils.GetCallerDesc(1))
	}
	if len(c.oneCondField) == 0 {
...
func (c *CacheRow[T]) Add2(ctx context.Context, cond TableConds, data interface{}) (interface{}, error) {
	if ctx.Value(utils.CtxKey_caller) == nil {
		ctx = context.WithValue(ctx, utils.CtxKey_caller, utils.GetCallerDesc(1))
	}
	// 检查data数据
...
// 使用该函数需要提前使用ConfigOneCondField配置相关内容
// 见AddM2说明
func (c *CacheRow[T]) AddM2OC(ctx context.Context, condValue interface{}, data map[string]interface{}) (interface{}, error) {
	if ctx.Value(utils.CtxKey_caller) == nil {
		ctx = context.WithValue(ctx, utils.CtxKey_caller, utils.GetCallerDesc(1))
	}
	if len(c.oneCondField) == 0 {
...
// -     若data中含有设置的自增字段 或者 条件字段 会忽略掉
// 返回值：自增ID(如果新增数据 int64类型), error
func (c *CacheRow[T]) AddM2(ctx context.Context, cond TableConds, data map[string]interface{}) (interface{}, error) {
	if ctx.Value(utils.CtxKey_caller) == nil {
		ctx = context.WithValue(ctx, utils.CtxKey_caller, utils.GetCallerDesc(1))
	}
	// 检查data数据
...
func (c *CacheRow[T]) _Add2(ctx context.Context, cond TableConds, dataInfo *utils.StructInfo) (interface{}, error) {
	// 检查条件变量
	key, err := c.checkCond(cond)
	if err != nil {
		return nil, err
...
func (c *CacheRow[T]) _Set(ctx context.Context, cond TableConds, dataInfo *utils.StructInfo, nc bool) (*T, interface{}, error) {
	...
	dest, err := c.redisSetGetToMysql(ctx, cond, key, dataInfo)
	if err == nil {
		return dest, nil, nil
	} else if err == ErrNullData {
		// 不处理执行下面的预加载
	...
	// 预加载 尝试从数据库中读取
	preData, incrValue, err := c.preLoad(ctx, cond, key, utils.If(nc, dataInfo, nil))
	if err != nil {
		return nil, nil, err
	}
...
func (c *CacheRow[T]) _Set2(ctx context.Context, cond TableConds, dataInfo *utils.StructInfo, nc bool) (interface{}, error) {
	...
	// 预加载 尝试从数据库中读取
	_, incrValue, err := c.preLoad(ctx, cond, key, utils.If(nc, dataInfo, nil))
	if err != nil {
		return nil, err
	}
	// 返回了自增，数据添加已经完成了，预加载的数据就是新增的
...
func (c *CacheRow[T]) redisSetToMysql(ctx context.Context, cond TableConds, key string, dataInfo *utils.StructInfo) error {
	...
	if cmd.Cmd.Err() == nil {
		// 同步mysql
		err := c.saveToMySQL(ctx, cond, dataInfo, key, func(err error) {
			if err != nil {
...
// 返回值*T
func (c *CacheRow[T]) redisSetGetToMysql(ctx context.Context, cond TableConds, key string, dataInfo *utils.StructInfo) (*T, error) {
	// 加锁
	unlock, err := c.saveLock(ctx, key)
	if err != nil {
		return nil, err
...
func (c *CacheRow[T]) redisModifyGetToMysql1(ctx context.Context, cond TableConds, key string, dataInfo, resInfo *utils.StructInfo) (*T, error) {
	...
	if cmd.Cmd.Err() == nil {
		...
		if err == nil {
			...
			err = c.saveToMySQL(ctx, cond, resInfo, key, func(err error) {
				if err != nil {
					c.redis.Del(ctx, key) // mysql错了 要删缓存
				}
			})
...
// error： 执行结果
func (c *CacheRow[T]) preLoad(ctx context.Context, cond TableConds, key string, ncInfo *utils.StructInfo) (*T, interface{}, error) {
	// 加锁
