...
// cond：查询条件变量
// 返回值：是T结构类型的指针列表
func (c *CacheRows[T]) Get(ctx context.Context, cond TableConds) ([]*T, error) {
	...
	err = c.redis.DoScript2(ctx, rowsGetScript, []string{key}, redisParams).BindSlice(&reply)
	if err == nil {
	...
	// 预加载 尝试从数据库中读取
	preData, _, err := c.preLoad(ctx, cond, key, false, 0, nil)
	if err == ErrNullData {
...
func (c *CacheRows[T]) GetOC(ctx context.Context, condValue interface{}) ([]*T, error) {
	if ctx.Value(utils.CtxKey_caller) == nil {
		ctx = context.WithValue(ctx, utils.CtxKey_caller, utils.GetCallerDesc(1))
	}
...
func (c *CacheRows[T]) Set(ctx context.Context, cond TableConds, data interface{}, nc bool) (interface{}, error) {
	...
	// 预加载 尝试从数据库中读取
	_, incrValue, err := c.preLoad(ctx, cond, key, nc, dataKeyValue, dataInfo)
	if err != nil {
		utils.LogCtx(log.Error(), ctx).Err(err).Msg("Set error")
		return nil, err
	}
...
func (c *CacheRows[T]) Modify(ctx context.Context, cond TableConds, data interface{}, nc bool) (interface{}, error) {
	...
	// 预加载 尝试从数据库中读取
	preData, incrValue, err := c.preLoad(ctx, cond, key, nc, dataKeyValue, dataInfo)
	if err != nil {
		utils.LogCtx(log.Error(), ctx).Err(err).Msg("Modify error")
		return nil, err
	}
...
// 按T来解析
func (c *CacheRows[T]) parseRedis(reply [][]interface{}) ([]*T, error) {
...
// error： 执行结果
func (c *CacheRows[T]) preLoad(ctx context.Context, cond TableConds, key string, nc bool, dataKeyValue interface{}, dataInfo *utils.StructInfo) ([]*T, interface{}, error) {
	// 根据是否要创建数据，来判断使用什么锁
	if nc {
		// 不存在要创建数据
		...
		// 查询mysql是否存在这条数据
		_, err = c.getFromMySQL(ctx, cond.Eq(c.dataKeyField, dataKeyValue)) // 此处用get函数
		...
		// 加载
		t, err := c.mysqlToRedis(ctx, cond, key)
...
func (c *CacheRows[T]) mysqlToRedis(ctx context.Context, cond TableConds, key string) ([]*T, error) {
