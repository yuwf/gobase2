...
type Cache struct {
	...
	gseq         utils.GroupSequence // 分组顺序执行任务
...
func (c *Cache) preLoadLock(ctx context.Context, key string) (func(), error) {
	...
	var lockKey strings.Builder
	if len(c.keyPrefix) > 0 {
		lockKey.WriteString(c.keyPrefix + "_")
	}
	lockKey.WriteString("lock_preLoad_")
	lockKey.WriteString(key)
	if len(c.keySuffix) > 0 {
		lockKey.WriteString("_" + c.keySuffix)
	}
	return c.redis.Lock(context.WithValue(ctx, goredis.CtxKey_nolog, 1), lockKey.String(), time.Second*8)
}
...
func (c *Cache) saveLock(ctx context.Context, key string) (func(), error) {
	...
	lockKey.WriteString("lock_save_")
	lockKey.WriteString(key)
	if len(c.keySuffix) > 0 {
		lockKey.WriteString("_" + c.keySuffix)
	}
...
func (c *Cache) addToMySQL(ctx context.Context, cond TableConds, dataInfo *utils.StructInfo) (int64, error) {
	...
	if len(c.incrementField) != 0 {
		...
		if incrementId == 0 {
			// 获取自增id
			err := c.incrementReids.DoScript2(ctx, incrScript, []string{IncrementKey}, c.TableName(), c.tableCount, c.tableIndex).Bind(&incrementId)
			if err != nil {
				return 0, err
			}
		}
	...
	for _, tag := range c.tableInfo.MySQLTags {
		...
		if dataInfo != nil {
			if at := dataInfo.FindIndexByTag(tag); at != -1 && dataInfo.Elemts[at].CanInterface() {
				// 如果是Time类型，且没有填充，忽略
				if t, ok := dataInfo.Elemts[at].Interface().(time.Time); ok && t.IsZero() {
					continue
				}
				fields = append(fields, tag)
	...
	if err != nil {
		...
		if len(c.incrementField) != 0 && utils.IsMatch("*Error 1062**Duplicate*PRIMARY*", err.Error()) {
			...
			if err2 == nil {
				...
				_, err := c.mysql.Exec(ctx, sqlStr.String(), args...)
				if err == nil {
					c.incrementReids.Do(ctx, "HSET", IncrementKey, c.TableName(), incrementId) // 保存下最大的key
...
// 保存到mysql
func (c *Cache) saveToMySQL(ctx context.Context, cond TableConds, destInfo *utils.StructInfo, key string, call func(err error)) error {
	var sqlStr strings.Builder
	...
	for i, tag := range destInfo.Tags {
		...
		// 如果是Time类型，且没有填充，忽略
		if t, ok := destInfo.Elemts[i].Interface().(time.Time); ok && t.IsZero() {
			continue
		}
		if num > 0 {
			sqlStr.WriteString(",")
